...
- `message = TRUE` allows messages to appear in output
...
- `message = FALSE` prevents messages appearing in output

```{r, message= FALSE, echo=TRUE, warning=TRUE}
library(ape)
```
...
--
<ol>
<li> Put references in a plain text file with the extension **.bib**, in **BibTex** format (People's advice: use [Zotero](https://www.zotero.org), my own advice use [Mendeley](https://www.mendeley.com/))
</ol>
```bibtex
*@article{Shea2014,
